...
/// @notice Minimal ERC-7984 stand-in for unit + integration tests where the live
///         cUSDT (`0x4E7B06D78965594eB5EF5414c357ca21E1554491`) is not available.
///         NOT FOR PRODUCTION. Real Sepolia deployment uses the canonical cUSDT.
contract MockConfidentialUSDT is IERC7984, ZamaEthereumConfig {
    ...
    /// @notice Test-only: mint cleartext amount to `to`. Real cUSDT mints via the wrapper.
    ...
    function _transfer(address from, address to, euint64 amount) internal returns (euint64 sent) {
        ...
        // Grant the returned ciphertext to the caller so it can keep using it
        // within the same transaction (matches canonical ERC-7984 behaviour).
        FHE.allowThis(sent);
